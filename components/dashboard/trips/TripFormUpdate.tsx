...
const formSchema = z.object({
  ...
  name: z.string(),
  price: z.number(),
  startRegister: z.coerce.date(),
  endRegister: z.coerce.date(),
  startTrip: z.coerce.date(),
  endTrip: z.coerce.date(),
  image: z.any().optional(),
});

const TripFormUpdate = ({ slug }: any) => {
  const [textEditor, setTextEditor] = useState("");

  const { data: tripDetailData, isSuccess } = useFetchData({
    queryKey: ["tripDetailData"],
  ...
  useEffect(() => {
    if (isSuccess) {
      setTextEditor(tripDetailData?.data.trip.description);
    }
  }, [tripDetailData?.data.trip.description, isSuccess]);

  const preLoadValues = {
    tourGuideId: tripDetailData?.data.trip.tourGuide.id,
    name: tripDetailData?.data.trip.name,
    price: tripDetailData?.data.trip.price,
    startRegister: tripDetailData?.data.trip.registrationPeriod.start,
    endRegister: tripDetailData?.data.trip.registrationPeriod.end,
    startTrip: tripDetailData?.data.trip.tripPeriod.start,
    endTrip: tripDetailData?.data.trip.tripPeriod.end,
  };
  ...
  const mutationUpdateTrip = useUpdateData({
    queryKey: "tripData",
    dataProtected: `trips/${slug}`,
  });
  ...
  const onSubmit = (data: FieldValues) => {
    ...
    mutationUpdateTrip.mutate(form);
  };
